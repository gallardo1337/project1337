...
const supportingActorFallbackImage = normalizeUrl(
  "/support-actor-fallback.webp",
  appBaseUrl
);
...
export function prioritizeTagIds(value, tags = []) {
  const mainTagIds = new Set(
    tags.filter((tag) => tag.is_main === true).map((tag) => String(tag.id))
  );

  return asIdList(value)
    .map((tagId, index) => ({ tagId, index, isMain: mainTagIds.has(tagId) }))
...
function actorImages(actor) {
  const profile = normalizeUrl(
    actor?.profile_image ||
      actor?.image_url ||
      actor?.photo_url ||
      actor?.avatar_url ||
  ...
  return {
    profile,
    transparent: normalizeUrl(actor?.transparent_image || null, appBaseUrl),
    cast: normalizeUrl(actor?.cast_image || null, appBaseUrl),
  };
}

function castMember(actor, fallbackImage = null) {
  if (!actor) return null;

  const images = actorImages(actor);
  const profileImage = images.profile || fallbackImage;

  return {
    id: String(actor.id),
    name: actor.name || actor.title || "Unbekannt",
    image_url: images.cast || images.transparent || profileImage,
    profile_image_url: profileImage,
    transparent_image_url: images.transparent,
    cast_image_url: images.cast,
...
export function buildTvLibraryPayload({
  ...
  const mappedMovies = movies.map((movie) => {
    ...
    const orderedTagIds = prioritizeTagIds(tagIds, tags);
    ...
    const supportCast = supportingActorIds
      .map((actorId) =>
        castMember(
          supportingActorMap.get(actorId),
          supportingActorFallbackImage
        )
      )
      .filter(Boolean);
  ...
  const mappedActors = actors
    .map((actor) => {
      ...
      return {
        id: String(actor.id),
        name: actor.name || actor.title || "Unbekannt",
        image_url: images.cast || images.transparent || images.profile,
  ...
  return {
    ...
    filters: {
      tags: tags
        ...
        .sort(
          (a, b) =>
            Number(b.is_main) - Number(a.is_main) || sortByName(a, b)
        ),
