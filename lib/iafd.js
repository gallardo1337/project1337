...
const SEARCH_PATH_PATTERN = /^\/results\.asp$/i;
...
export function validateIafdSearchUrl(value) {
  let parsed;
  try {
    parsed = new URL(String(value || "").trim(), IAFD_ORIGIN);
  } catch {
    throw new IafdError("Die IAFD-Suchadresse ist ungültig.", "INVALID_IAFD_URL", 400);
  }

  if (
    !new Set(["iafd.com", "www.iafd.com"]).has(parsed.hostname.toLowerCase()) ||
    !SEARCH_PATH_PATTERN.test(parsed.pathname)
  ) {
    throw new IafdError(
      "Der Browser hat keine gültige IAFD-Suchseite übergeben.",
      "INVALID_IAFD_SEARCH_URL",
      400
    );
  }
...
export function iafdHtmlLooksUnavailable(html) {
...
export async function fetchIafdHtml(url, expectedType = "title") {
  ...
  try {
    const html = await fetchIafdHtmlThroughBridge(safeUrl);
    if (iafdHtmlLooksUnavailable(html)) {
      throw new IafdError(
        "Die NAS-Bridge wurde von IAFD nicht durch die Browserprüfung gelassen.",
        "IAFD_CHALLENGE",
        503
      );
    }
    ...
    if (error instanceof IafdBridgeClientError) {
      if (error.code === "IAFD_BRIDGE_NOT_CONFIGURED") {
        throw new IafdError(
          "Die IAFD-NAS-Bridge ist in dieser Umgebung noch nicht verbunden.",
          error.code,
          503
        );
      }
      if (error.code === "IAFD_BRIDGE_TIMEOUT" || error.code === "NAVIGATION_TIMEOUT") {
        throw new IafdError(
      ...
      if (new Set(["AUTH_REQUIRED", "BAD_SIGNATURE", "TIMESTAMP_EXPIRED"]).has(error.code)) {
        throw new IafdError(
          "Die Verbindung zur IAFD-NAS-Bridge konnte nicht authentifiziert werden.",
          "IAFD_BRIDGE_AUTH_FAILED",
          503
        );
      }
      throw new IafdError(
        error.message || "Die IAFD-NAS-Bridge konnte die Seite nicht laden.",
        error.code,
        error.status
      );
    }
    throw new IafdError(
      "Die IAFD-NAS-Bridge konnte die Seite nicht verarbeiten.",
      "IAFD_BRIDGE_FAILED",
      503
    );
  }
...
export function buildIafdSearchUrl(query) {
  ...
  return validateIafdSearchUrl(url.toString());
}
