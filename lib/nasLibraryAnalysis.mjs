...
function inferFolderQuality(path) {
  ...
  return parts.some((part) => part === "4k") ? "4K" : "Nicht 4K";
}
...
const PERFORMER_SORT_KEYS = new Set([
  "name",
  "nas_files",
  "database_movies",
  "exact",
...
export function sortNasPerformers(
  performers,
  { key = "nas_files", direction = "desc" } = {}
) {
  const safeKey = PERFORMER_SORT_KEYS.has(key) ? key : "nas_files";
  const factor = direction === "asc" ? 1 : -1;

  return [...(Array.isArray(performers) ? performers : [])].sort((left, right) => {
    let comparison;
    ...
    if (safeKey === "name") {
      ...
      comparison = numberValue(left?.[safeKey]) - numberValue(right?.[safeKey]);
    }

    if (comparison) return comparison * factor;

    const nasFileComparison =
      numberValue(right?.nas_files) - numberValue(left?.nas_files);
    if (safeKey !== "nas_files" && nasFileComparison) return nasFileComparison;
...
export function analyzeNasLibrary({ inventory, movies, actors, resolutions }) {
  ...
  reportFiles.forEach((file) => {
    ...
    const quality = file.inferred_quality;
  ...
  const performers = sortNasPerformers(
    [...performerMap.values()].map((performer) => ({
      key: performer.key,
